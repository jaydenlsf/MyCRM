...
namespace MyPlugins
{
    public class DuplicateChecker : IPlugin
    {
                ...
                context.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    ...
                    if(contact.Attributes.Contains("emailaddress1"))
                    {
                        ...
                        // SELECT * FROM contact WHERE emailaddress == 'email'
                        ...
                        {
                            throw new InvalidPluginExecutionException("Email address already exist.");
                        }
                    }
                    if(contact.Attributes.Contains("lastname"))
                    {
                        ...
                        QueryExpression query = new QueryExpression("contact");
                        query.ColumnSet = new ColumnSet(new string[] { "lastname" });
                        query.Criteria.AddCondition("lastname", ConditionOperator.Equal, lastname);
                        EntityCollection collection = service.RetrieveMultiple(query);

                        if (collection.Entities.Count > 0)
                        {
                            throw new InvalidPluginExecutionException("Last name already exist.");
                        }
                    }
